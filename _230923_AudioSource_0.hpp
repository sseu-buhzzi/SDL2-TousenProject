...
namespace AudioSource {
    ...
    unsigned short const audio_samples{0x0100U};
    unsigned char audio_sample_width{0x02U};
    int const audio_freq{44100U};
    ...
    ::std::string const audio_files[]{
        ...
        "D:\\Sseu\\Tryingssiuh\\Ssianxmuh\\_230515_STGs\\_230824_STG\\src\\ShootSound.mem",
    ...
    inline void audio_cbfunc(void *audio_ptr, ::Uint8 *audio_stream, int length) {
        ::AudioSource::Audio *audio{reinterpret_cast<::AudioSource::Audio *>(audio_ptr)};
        if (audio->offset + length >= audio->length) {
            length = audio->length - audio->offset;
        }
        for (unsigned int i{0U}; i < length; ++i) {
            *audio_stream++ = audio->data[audio->offset++];
        }
        if (length / 2 != ::AudioSource::audio_samples) {
            audio->offset = 0U;
        }
    }
    inline void source_init() {
        for (unsigned int i{0U}; i < ::AudioSource::audio_count; ++i) {
            ...
            unsigned int audio_id(::AudioSource::audio_list.size() - 1);
    ...
    inline void play_audio(unsigned int audio_id) {
// long long int lt;
...
// if (audio_id == ::AudioSource::audio_ids["graze_sound"]) { long long int ct{::std::chrono::system_clock::now().time_since_epoch().count()}; ::std::cout << ct - lt << '\n'; lt = ct; }
        audio_spec.userdata = ::AudioSource::audio_list.data() + audio_id;
// if (audio_id == ::AudioSource::audio_ids["graze_sound"]) { long long int ct{::std::chrono::system_clock::now().time_since_epoch().count()}; ::std::cout << ct - lt << '\n'; lt = ct; }
        ::AudioSource::audio_list[audio_id].device_id = ::SDL_OpenAudioDevice(nullptr, 0, &audio_spec, nullptr, 0);
// if (audio_id == ::AudioSource::audio_ids["graze_sound"]) { long long int ct{::std::chrono::system_clock::now().time_since_epoch().count()}; ::std::cout << ct - lt << '\n'; lt = ct; }
        ::SDL_PauseAudioDevice(::AudioSource::audio_list[audio_id].device_id, 0);
// if (audio_id == ::AudioSource::audio_ids["graze_sound"]) { long long int ct{::std::chrono::system_clock::now().time_since_epoch().count()}; ::std::cout << ct - lt << '\n'; lt = ct; ::std::cin.get(); }
    }
    inline void pause_audio(unsigned int audio_id) {
        ::SDL_PauseAudioDevice(::AudioSource::audio_list[audio_id].device_id, 1);
        ::SDL_CloseAudioDevice(::AudioSource::audio_list[audio_id].device_id);
    }
    inline void rewind_audio(unsigned int audio_id) {
        ::AudioSource::audio_list[audio_id].offset = 0U;
    }
    inline bool is_audio_playing(unsigned int audio_id) {
        return ::SDL_GetAudioDeviceStatus(::AudioSource::audio_list[audio_id].device_id) == SDL_AUDIO_PLAYING;
    }
